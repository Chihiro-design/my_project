...
class Example(object):
    ...
    def __init__(self, idx, source, similar_source, target):
        self.idx = idx
        self.source = source
        ...
        for idx, line in enumerate(f):
            ...
            examples.append(
                Example(
                        idx = idx,
                        source=code,
                        similar_source=similar_docstring,
                        target = nl,
                        ) 
            )
    return examples
...
class InputFeatures(object):
    ...
    def __init__(self, example_id, source_ids, similar_source_ids, target_ids, source_mask, similar_source_mask, target_mask):
        self.example_id = example_id
        self.source_ids = source_ids
        self.similar_source_ids=similar_source_ids
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        padding_length = args.max_source_length - len(similar_source_ids)
        similar_source_ids+=[tokenizer.pad_token_id]*padding_length
        similar_source_mask+=[0]*padding_length
        ...
        '''if example_index < 5:
            if stage=='train':
                ...
                logger.info("similar_source_tokens: {}".format([x.replace('\u0120','_') for x in similar_source_tokens]))
                logger.info("similar_source_ids: {}".format(' '.join(map(str, similar_source_ids))))
                logger.info("similar_source_mask: {}".format(' '.join(map(str, similar_source_mask))))
...
def main():
    ...
    if args.do_train:
        ...
        all_similar_source_ids = torch.tensor([f.similar_source_ids for f in train_features], dtype=torch.long)
        all_similar_source_mask = torch.tensor([f.similar_source_mask for f in train_features], dtype=torch.long)
        all_target_ids = torch.tensor([f.target_ids for f in train_features], dtype=torch.long)
        all_target_mask = torch.tensor([f.target_mask for f in train_features], dtype=torch.long)    
        train_data = TensorDataset(all_source_ids,all_source_mask,all_similar_source_ids,all_similar_source_mask,all_target_ids,all_target_mask)
